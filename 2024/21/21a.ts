...
for (let i = 0; i < lines.length; i++) {
    ...
    for (const r1 of robot1) {
        const fullSequence = sequenceToSequence(sequenceToSequence(r1));
        if (fullSequence.length < shortestLen) {
            shortestLen = fullSequence.length;
            shortestSequence = fullSequence;
        }
    }
...
function directionalButtonPos(button: string): [number, number] {
    switch (button) {
        case '^': return [0, 1];
        case 'A': return [0, 2];
        case '<': return [1, 0];
