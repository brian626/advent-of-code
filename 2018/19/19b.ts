...
let count = 0;
let prevRegisterZeroValue = registers[0];
while (true) {
    // if (count % 10000 === 0) { console.log(`${count}: ${registers}`); }
    if (registers[0] !== prevRegisterZeroValue) {
        console.log(`${count}: ${registers}`);
        prevRegisterZeroValue = registers[0];
    }
    count++;
