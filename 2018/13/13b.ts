...
while (true) {
    ...
    let cartsToRemove: Set<number> = new Set<number>();
        ...
        for (let i = 0; i < carts.length; i++) {
            for (let j = i + 1; j < carts.length; j++) {
                ...
                if (carts[i].y === carts[j].y && carts[i].x === carts[j].x) {
                    debug(`collision at [${carts[i].x},${carts[i].y}] between carts ${i} and ${j}`);
                    cartsToRemove.add(i);
                    cartsToRemove.add(j);
                }
            }
    ...
    if (cartsToRemove.size > 0) {
        debug(`carts before removal`);
        // console.log(carts);
        const ctr = Array.from(cartsToRemove).sort((a, b) => b - a);
        for (const c of ctr) {
            debug(`removing cart ${c}`);
            carts.splice(c, 1);
        }
        debug(`carts after removal`);
        // console.log(carts);
    }
