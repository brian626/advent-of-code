    ...
    for (let r = claim.top; r < claim.top + claim.height; r++) {
        ...
        for (let c = claim.left; c < claim.left + claim.width; c++) {
            if (grid[r][c] === undefined) {
                ...
                // console.log(`claim ${claim.id} overlaps at [${r},${c}] with claim ${grid[r][c]}`);
                const overlapClaim = claims.filter(x => x.id === grid[r][c])[0];
                if (overlapClaim) { overlapClaim.overlap = true; }
                claim.overlap = true;
                grid[r][c] = 'X';
            }
        }
...
// console.log(grid);
console.log(claims.filter(x => !x.overlap)[0].id);
