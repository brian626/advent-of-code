...
// Snailfish numbers aren't like regular numbers. Instead, every snailfish number is a pair - an ordered
// list of two elements. Each element of the pair can be either a regular number or another pair.
...
// and right parameters of the addition operator. For example, [1,2] + [[3,4],5] becomes [[1,2],[[3,4],5]].
...
// [[[[[9,8],1],2],3],4] becomes [[[[0,9],2],3],4] (the 9 has no regular number to its left,
//    so it is not added to any regular number).
// [7,[6,[5,[4,[3,2]]]]] becomes [7,[6,[5,[7,0]]]] (the 2 has no regular number to its right,
//    and so it is not added to any regular number).
...
class SnailfishNumber {
    constructor(s: string | null) {
        if (s === null) {
            ...
            // Find the middle comma and recursively split around it.
            let depth = 0;
            for (let i = 0; i < s.length; i++) {
                if (s[i] === '[') { depth += 1; }
                else if (s[i] === ']') { depth -= 1; }
                else if (s[i] === ',' && depth === 1) {
                    // console.log(`left: ${s.slice(1,i)}`);
                    // console.log(`right: ${s.slice(i+1, s.length - 1)}`);
                    this.left = new SnailfishNumber(s.slice(1, i));
                    this.right = new SnailfishNumber(s.slice(i+1, s.length - 1));
                    break;
                }
            }
    ...
    toStringHelper(x: SnailfishNumber): string {
        let s = '';
        if (x.val === null) { s += '['; }
        else { s += `${x.val}`; }
        ...
        if (x.left !== null) { leftPart = this.toStringHelper(x.left); }
        if (x.right !== null) { rightPart = this.toStringHelper(x.right); }
...
const numbers: string[] = new Array();
...
console.log(`first string: ${numbers[0]}`);
let sum = new SnailfishNumber(numbers.shift());
console.log(`first number: ` + sum.toString());

while (numbers.length > 0) {
    const n = new SnailfishNumber(numbers.shift());
    console.log(`\n  adding <${sum.toString()}> and <${n.toString()}>`);
    sum = add(sum, n);
    console.log(`  sum after addition: <${sum}>`);
    sum = reduce(sum);
}
console.log(`\nsum is: `+ sum.toString());
...
function add(x: SnailfishNumber, y: SnailfishNumber): SnailfishNumber {
    const sum = new SnailfishNumber(null);
    sum.left = x;
    sum.right = y;
    return sum;
}
...
function dfsToMaxDepth(x: SnailfishNumber, visited: Map<SnailfishNumber, boolean>, depth: number, maxDepth: number): SnailfishNumber {
    visited.set(x, true);
    ...
    if (x.left) {
        if (visited.get(x.left) === undefined || visited.get(x.left) === false) {
            return dfsToMaxDepth(x.left, visited, depth + 1, maxDepth);
        }
    }

    if (x.right) {
        if (visited.get(x.right) === undefined || visited.get(x.right) === false) {
            return dfsToMaxDepth(x.right, visited, depth + 1, maxDepth);
        }
    }
...
// Then, the entire exploding pair is replaced with the regular number 0.
//
function reduce(x: SnailfishNumber): SnailfishNumber {
    while (true) {
        // Is there a pair nested inside four pairs? (Do a depth-first search up to level 5.)
        const visited: Map<SnailfishNumber, boolean> = new Map<SnailfishNumber, boolean>();
        const pairToExplode = dfsToMaxDepth(x, visited, 0, 4);
        if (pairToExplode) {
            console.log(`pair ${pairToExplode} needs to explode`);
            // continue;
        }

        // Are there any regular numbers 10 or greater? (Do a full dfs.)
        if (false) {
            continue;
        }
...
// The magnitude of a pair is 3 times the magnitude of its left element plus 2 times the magnitude of its
// right element. The magnitude of a regular number is just that number.
function getMagnitude(n: SnailfishNumber): number {
    if (n.val !== null) { return n.val; }
