...
// ocean floor; sensors indicate a massive underground cave system just beyond where they're aiming!
...
// choose any horizontal position to align them all on, but the one that costs the least fuel is horizontal position 2:
...
for (let i = minPos; i <= maxPos; i++) {
    const fuel = positions.map(x => Math.abs(x - i)).reduce((a,b) => a + b, 0);
