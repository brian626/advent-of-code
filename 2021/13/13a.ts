...
// paper! The transparent paper is marked with random dots and includes instructions on
...
// y, increases downward. So, the coordinate 3,0 is to the right of 0,0, and the
// coordinate 0,7 is below 0,0. The coordinates in this example form the following
...
// first fold. After the first fold in the example above, 17 dots are visible - dots
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (parts.length === 2) {
        ...
        paperWidth = Math.max(paperWidth, dot.x);
        paperHeight = Math.max(paperHeight, dot.y);
    } else {
        parts = lines[i].split(' ');
...
if (firstFold.direction === 'y') {
    // console.log('folding up');
    // Fold the bottom section up.
    for (let i = 0; i < dots.length; i++) {
        const dot = dots[i];
        if (dot.y > firstFold.position) {
            // console.log(`  changing dot [${dot.x}, ${dot.y}] to [${dot.x}, ${paperHeight - dot.y}]`);
            dot.y = (paperHeight - dot.y);
        } else {
            // console.log(`  NOT changing dot [${dot.x}, ${dot.y}]`);
        }
    }

    paperHeight -= (firstFold.position + 1);
} else {
    // Fold the right section to the left.
    for (let i = 0; i < dots.length; i++) {
        const dot = dots[i];
        if (dot.x > firstFold.position) {
            dot.x = (paperWidth - dot.x);
        }
    }

    paperWidth -= (firstFold.position + 1);
}

// Count the visible dots.
const visibleDots: Dot[] = new Array();
dots.forEach(d => {
    let alreadyHave = false;
    for (let i = 0; i < visibleDots.length; i++) {
        if (visibleDots[i].x === d.x && visibleDots[i].y === d.y) {
            alreadyHave = true;
            break;
        }
    }
