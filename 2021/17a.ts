...
// by 1 if it is greater than 0, increases by 1 if it is less than 0, or does not change if it is already 0.
...
// any step, the probe's x position is at least 20 and at most 30, and the probe's y position
...
// might as well do it with style. How high can you make the probe go while still reaching the target area?

// In the above example, using an initial velocity of 6,9 is the best you can do, causing the
// probe to reach a maximum y position of 45. (Any higher initial y velocity causes the probe
// to overshoot the target area entirely.)
...
for (let initialProbeVelocityX = 1; initialProbeVelocityX < xMax; initialProbeVelocityX++) {
    for (let initialProbeVelocityY = 0; initialProbeVelocityY < 2000; initialProbeVelocityY++) {
