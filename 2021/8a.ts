...
// segments b and g are turned on: the only digit that uses two segments is 1, so it must mean
// segments c and f are meant to be on. With just that information, you still can't tell which
// wire (b/g) goes to which segment (c/f). For that, you'll need to collect more information.
...
// input). Using the signal patterns, you should be able to work out which pattern corresponds to which digit.
...
// output values (the part after | on each line), in the above example, there are 26 instances of
