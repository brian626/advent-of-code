...
// each position is used. So, the epsilon rate is 01001, or 9 in decimal. Multiplying the gamma rate (22) by the epsilon
...
const file = readFileSync('./3a.input', 'utf-8');
...
for (let i = 0; i < lines.length; i++) {
    const digits: string[] = lines[i].split('');

    for (let j = 0; j < digits.length; j++) {
        if (counts[j] === undefined) { counts[j] = 0; }
        counts[j] += digits[j] === '1' ? 1 : 0;
    }
}
