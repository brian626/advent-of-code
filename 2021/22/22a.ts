...
// The third step (off x=9..11,y=9..11,z=9..11) turns off a 3x3x3 cuboid that overlaps partially with some
// cubes that are on, ultimately turning off 8 cubes:
...
// The final step (on x=10..10,y=10..10,z=10..10) turns on a single cube, 10,10,10. After this last step,
// 39 cubes are on.
...
const reactor: number[][][] = new Array();
for (let x = 0; x <= 100; x++) {
    reactor[x] = new Array();
    for (let y = 0; y <= 100; y++) {
        reactor[x][y] = new Array();
        for (let z = 0; z <= 100; z++) {
            reactor[x][y][z] = 0;
        }
    }
...
for (let i = 0; i < lines.length; i++) {
    ...
    for (let x = xMin + 50; x <= xMax + 50; x++) {
        for (let y = yMin + 50; y <= yMax + 50; y++) {
            for (let z = zMin + 50; z <= zMax + 50; z++) {
                reactor[x][y][z] = (toggle === 'on') ? 1 : 0;
            }
        }
    }
}
...
for (let x = 0; x <= 100; x++) {
    for (let y = 0; y <= 100; y++) {
        for (let z = 0; z <= 100; z++) {
            if (reactor[x][y][z] === 1) { numOnCubes += 1; }
        }
    }
}

console.log(numOnCubes);
