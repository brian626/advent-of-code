...
for (let i = 0; i < SECONDS; i++) {
    ...
    const maxDistance = Math.max(...reindeer.map(x => x.travelDistance));

    for (const r of reindeer) {
        if (r.travelDistance === maxDistance) {
            r.score += 1;
        }
    }
}

console.log(reindeer.map(x => `${x.name} has a score of ${x.score} and is ${x.isFlying ? 'flying' : 'resting'}.`));

console.log(reindeer.sort((a, b) => b.score - a.score)[0]);
