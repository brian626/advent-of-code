...
for (let i = 0; i < lines.length; i++) {
    ...
    const nextNextPassword = generate(nextPassword.split('').map(x => x.charCodeAt(0) - 'a'.charCodeAt(0)));
    console.log(`The next password after ${nextPassword} is ${nextNextPassword}`);
}
