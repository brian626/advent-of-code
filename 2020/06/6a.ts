...
const groups: Set<string>[] = new Array();
let groupNum = 0;

for (let i = 0; i < lines.length; i++) {
    if (lines[i].length === 0) { groupNum += 1; continue; }

    if (groups[groupNum] === undefined) { groups[groupNum] = new Set<string>(); }

    const questions = lines[i].split('');
    questions.forEach(q => {
        groups[groupNum].add(q);
    })
}
