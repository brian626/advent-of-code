...
for (let i = 0; i < adapters.length; i++) {
    if (adapters[i] === currentJoltage + 1) { numOneJoltDifferences += 1; }
    else if (adapters[i] === currentJoltage + 3) { numThreeJoltDifferences += 1; }

    currentJoltage = adapters[i];
}

console.log(numOneJoltDifferences * numThreeJoltDifferences);
