...
const buses: number[] = lines[1].split(',').filter(x => x !== 'x').map(x => parseInt(x));
const busDepartures: number[] = new Array();
...
for (let i = 0; i < buses.length; i++) {
    ...
    busDepartures[i] = time;
}

// console.log(busDepartures);

const earliestDeparture = Math.min(...busDepartures);
// console.log(earliestDeparture);

const earliestBus = buses[busDepartures.indexOf(earliestDeparture)];

console.log(earliestBus * (earliestDeparture - earliestTimestamp));
