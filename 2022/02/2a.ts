...
// a winner for that round is selected: Rock defeats Scissors, Scissors defeats Paper, and Paper defeats
...
// play: A for Rock, B for Paper, and C for Scissors. The second column--" Suddenly, the Elf is called
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (choices[0] === 'A') {
        if      (choices[1] === 'X') { score += (1 + 3); }
        else if (choices[1] === 'Y') { score += (2 + 6); }
        else if (choices[1] === 'Z') { score += (3 + 0); }
    } else if (choices[0] === 'B') {
        if      (choices[1] === 'X') { score += (1 + 0); }
        else if (choices[1] === 'Y') { score += (2 + 3); }
        else if (choices[1] === 'Z') { score += (3 + 6); }
    } else if (choices[0] === 'C') {
        if      (choices[1] === 'X') { score += (1 + 6); }
        else if (choices[1] === 'Y') { score += (2 + 0); }
        else if (choices[1] === 'Z') { score += (3 + 3); }
    }
}
