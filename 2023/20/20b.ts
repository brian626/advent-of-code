...
for (const m of modules) {
    for (const d of m.destinations) {
        const destinationModule = modules.find(x => x.name === d); // findModule(d);
...
const BUTTON_PUSHES = 1000000000;
let pushesToFinish = 0;

for (let i = 0; i < BUTTON_PUSHES; i++) {
    if (pushesToFinish > 0) { break; }
    ...
    while (pulses.length > 0) {
        ...
        if (p.destination.name === 'rx') {
            if (p.type === PulseType.Low) {
                console.log(`rx got a ${p.type === PulseType.Low ? 'low' : 'high'} pulse`);
                pushesToFinish = i + 1;
                break;
            }
        }
