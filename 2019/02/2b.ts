...
// the opcode; 2, 3, and 4 are the parameters. The instruction 99 contains only an opcode and has no parameters.
...
while (true) {
    ...
    for (let i = 0; i < lines.length; i += 4) {
        ...
        const opcode = parseInt(lines[i]);
        const inputPos1 = parseInt(lines[i+1]);
        const inputPos2 = parseInt(lines[i+2]);
        const outputPos = parseInt(lines[i+3]);

        if (opcode == 1) {
            const result = parseInt(lines[inputPos1]) + parseInt(lines[inputPos2]);
            lines[outputPos] = result.toString();
        } else if (opcode == 2) {
            const result = parseInt(lines[inputPos1]) * parseInt(lines[inputPos2]);
            lines[outputPos] = result.toString();
        } else {
