...
// just calculated as the input mass and repeat the process, continuing until a fuel requirement is zero or negative. For example:
...
// into account the mass of the added fuel? (Calculate the fuel requirements for each module separately, then add them all up at the end.)
...
for (let i = 0; i < lines.length; i++) {
    ...
    totalFuel += calculateFuel(parseInt(lines[i])); //Math.floor(parseInt(lines[i]) / 3) - 2;
}
...
function calculateFuel(mass: number): number {
    ...
    const fuel = Math.floor(mass / 3);
    if (fuel > 2) {
        // console.log(`recursing on ${fuel - 2}`);
        return fuel - 2 + calculateFuel(fuel - 2);
    } else {
        // console.log(`returning ${fuel}`);
        return 0;
    }
}
